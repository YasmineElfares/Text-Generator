...
--docs = ["the man is the man. he is great","the man saw the saw","the man man."]
--docs = ["the man is the man. he is great","the man saw the saw"]
--docs = ["kontich is a municipality in the belgian province of antwerp.", "in 2014, 20859 people lived there.", "it is at 51 degree 08 north, 04 degree 26 east.", "kortemark"]
...
wordToken l = filter(/="")(split2 l)

wordTokenList :: [String] -> [String]
wordTokenList [[]] = []
wordTokenList l = concat(map wordToken l)
	
uniqueBigrams :: [String] -> [(String,String)]
uniqueBigrams [x] = []
uniqueBigrams (x:xs) = if not(elem (x,head xs) res) then concat[(x,head xs):uniqueBigrams(xs)]
                       else uniqueBigrams(xs)
                       where res = uniqueBigrams(xs)
					   
uniqueTrigrams :: [String] -> [(String,String,String)]
uniqueTrigrams [x,y] = []
uniqueTrigrams (x:xs) = if not(elem (x,head xs,head (tail xs)) res) then 
                        concat[(x,head xs, head (tail xs)):uniqueTrigrams(xs)]
						else uniqueTrigrams(xs)
                        where res = uniqueTrigrams(xs)

occ a [] = 0					
occ a (x:xs) = if a == x then 1 + occ a (xs)
               else occ a (xs)


bigramsFreq :: Num a => [String] -> [((String,String),a)]
bigramsFreqHelper [x] = []
bigramsFreqHelper (x:xs) = concat[(x,head xs):bigramsFreqHelper(xs)]
	
bigramsFreq [x] = []
bigramsFreq (x:xs) = ((x, head xs),occurred):(filter (/= ((x,head xs),(occurred)))(filter (/= ((x,head xs),(occurred-1))) all))
		    		 where {occurred = occ (x,head xs) (bigramsFreqHelper (x:xs));
			              all = ((x,head xs),occurred):bigramsFreq(xs);}
						  
						  
trigramsFreq:: Num a => [String] -> [((String,String,String),a)]
trigramsFreqHelper [x,y] = []
trigramsFreqHelper (x:xs) = concat[(x,head xs,head (tail xs)):trigramsFreqHelper(xs)]
	
trigramsFreq [x,y] = []
trigramsFreq (x:xs) = ((x,head xs,head (tail xs)),occurred):(filter (/= ((x,head xs,head (tail xs)),(occurred)))(filter (/= ((x,head xs,head (tail xs)),(occurred-1))) all))
		    		 where {occurred = occ (x,head xs,head (tail xs)) (trigramsFreqHelper (x:xs));
			               all = ((x,head xs,head (tail xs)),occurred):trigramsFreq(xs);}					  
...
generateOneProb a [] = 0
generateOneProb ((w1,w2,w3),o1) (((x,y),o):xs) = if w1 == x && w2 == y then o1/o
                                                else generateOneProb ((w1,w2,w3),o1) xs
					
genProbPairs :: Fractional a => [((String,String,String),a)] -> [((String,String),a)] -> [((String,String,String),a)]
genProbPairs [] l = []
genProbPairs (((w1,w2,w3),o1):xs) l = ((w1,w2,w3),generateOneProb ((w1,w2,w3),o1) l):(genProbPairs xs l)
						  

			
getIndexOf2 a [] = error "element does not exist" 
getIndexOf2 numb (x:xs) = if numb == 0 then x
                         else getIndexOf2 (numb-1) xs
						 
 --generates all possible words
generateNextWord :: (Ord a, Fractional a) => [String] -> [((String,String,String),a)] -> String
generateNextWordHelper _ [] = []
generateNextWordHelper [x,y] (((w1,w2,w3),p):xs) = if x == w1 && y == w2 && p > 0.03 then w3:(generateNextWordHelper ([x,y]) xs)
		                                           else generateNextWordHelper ([x,y]) xs			
generateNextWord [x,y] l = if all ==[] then error("Sorry, it is not possible to infer from current database")
							else getIndexOf2 (randomZeroToX ((length2 all) -1)) all
                           where all = generateNextWordHelper [x,y] l
...
get2words [x]= []
get2words (x:xs) = if length2 (x:xs) == 2 then (x:xs)
                   else get2words xs

generateText str 0 = str
generateText str number = generateText str2 (number-1)
                          where { 
							  s1 = wordToken str;
						      all = wordTokenList docs;
                              freqtriples= trigramsFreq all;
                              freqPairs = bigramsFreq all;
                              prob= genProbPairs freqtriples freqPairs;
							  str2 = str ++ " " ++ generateNextWord(get2words s1) prob;}
